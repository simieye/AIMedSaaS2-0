...
export default function DepartmentalAIDiagnosis(props) {
  ...
  const handleTabChange = value => {
    ...
    toast({
      title: "页面切换",
      description: `已切换到${value === 'workspace' ? 'AI诊断工作台' : value === 'history' ? '诊断历史' : '医学知识库'}页面`
    });
  };
        ...
        <div className="max-w-7xl mx-auto p-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">各科室智能诊断与医学知识管理</p>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                <SearchBar onSearch={handleSearch} placeholder="搜索诊断记录、医学文献..." />
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6">
            ...
            <TabsContent value="workspace" className="space-y-6">
              {isLoading ? <LoadingOverlay /> : <AIDiagnosisWorkspace $w={$w} />}
            </TabsContent>

            <TabsContent value="history" className="space-y-6">
              {isLoading ? <LoadingOverlay /> : <DiagnosisHistory $w={$w} />}
            </TabsContent>

            <TabsContent value="knowledge" className="space-y-6">
              {isLoading ? <LoadingOverlay /> : <MedicalKnowledgeBase $w={$w} />}
            </TabsContent>
          </Tabs>
        </div>
      </ErrorBoundary>
