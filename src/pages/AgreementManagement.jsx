...
export default function AgreementManagement(props) {
  ...
  const handleTabChange = value => {
    ...
    toast({
      title: "页面切换",
      description: `已切换到${value === 'list' ? '协议列表' : value === 'details' ? '协议详情' : value === 'documents' ? '文档管理' : '模板管理'}页面`
    });
  };
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">合作伙伴协议和文档管理</p>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                <SearchBar placeholder="搜索协议..." />
                <NotificationBell />
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6">
            ...
            <TabsContent value="details" className="space-y-6">
              {selectedAgreement ? <AgreementDetails $w={$w} agreement={selectedAgreement} onEdit={handleEditAgreement} /> : <EmptyState icon={FileContract} title="选择协议" description="请从协议列表中选择一个协议查看详情" />}
            </TabsContent>
            ...
            <TabsContent value="templates" className="space-y-6">
              <Card>
                ...
                <CardContent>
                  <EmptyState icon={Layout} title="模板管理" description="协议模板管理功能正在开发中..." />
                </CardContent>
              </Card>
            </TabsContent>
          </Tabs>
          ...
          <Modal isOpen={showForm} onClose={() => setShowForm(false)} title={selectedAgreement ? '编辑协议' : '新建协议'}>
            <AgreementForm agreement={selectedAgreement} onSave={handleSaveAgreement} onCancel={() => setShowForm(false)} />
          </Modal>
        </div>
      </ErrorBoundary>
    </div>;
