...
export default function UnifiedPortal(props) {
  ...
  const handleTabChange = value => {
    ...
    toast({
      title: "页面切换",
      description: `已切换到${value === 'dashboard' ? '仪表板' : value === 'patients' ? '患者管理' : value === 'doctors' ? '医生管理' : value === 'api-keys' ? 'API密钥' : '设置'}页面`
    });
  };
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">统一管理门户</h1>
                  <p className="text-gray-600">智能医疗AI综合管理平台</p>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                <SearchBar onSearch={handleSearch} placeholder="搜索患者、医生或API密钥..." />
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6">
            ...
            <TabsContent value="settings" className="space-y-6">
              <Card>
                ...
                <CardContent>
                  <EmptyState icon={Settings} title="设置页面" description="系统设置功能正在开发中..." />
                </CardContent>
              </Card>
            </TabsContent>
          </Tabs>
